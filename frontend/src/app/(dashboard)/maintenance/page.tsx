...
import { Wrench, Calendar, Plus, X } from "lucide-react";
...
export default function MaintenancePage() {
  ...
  // Only vehicles not retired can go to maintenance
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="border-b border-slate-800 bg-slate-900/30">
                ...
                <th className="p-4 text-xs font-semibold uppercase tracking-wider text-slate-400">Description</th>
                <th className="p-4 text-xs font-semibold uppercase tracking-wider text-slate-400">Start Date</th>
                ...
                logs.map((log) => {
                  ...
                  return (
                    <tr key={log.id} className="hover:bg-slate-900/30 transition-colors">
                      ...
                      <td className="p-4 text-slate-400 text-sm max-w-xs truncate">{log.description || "—"}</td>
                      <td className="p-4 font-mono text-slate-300 text-sm">{log.start_date}</td>
                      <td className="p-4 font-mono text-amber-400 font-bold text-sm">{formatCurrency(log.cost)}</td>
                      <td className="p-4 text-right">
                        <span className={`inline-block text-[10px] uppercase font-bold tracking-wider px-2 py-1 rounded-full border ${
                          log.active 
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative w-full max-w-lg bg-slate-900 border border-slate-800 rounded-3xl p-8 shadow-2xl flex flex-col relative overflow-hidden animate-slide-in">
            ...
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-xl font-bold text-slate-100">Log Maintenance Record</h2>
            ...
            <form onSubmit={handleCreateLog} className="space-y-4">
              ...
              {/* Title */}
              <div className="space-y-1.5">
                <label className="text-xs font-semibold text-slate-400 uppercase tracking-wider">Maintenance Title</label>
                <input
                  type="text"
                  value={title}
                  onChange={(e) => setTitle(e.target.value)}
                  className="w-full bg-slate-950 border border-slate-800 text-slate-200 text-sm px-4 py-2.5 rounded-xl focus:outline-none focus:border-indigo-500"
                  placeholder="e.g. Engine Oil & Oil Filter Service"
                />
              ...
              {/* Description */}
              <div className="space-y-1.5">
                <label className="text-xs font-semibold text-slate-400 uppercase tracking-wider">Additional details</label>
                <textarea
                  value={description}
                  onChange={(e) => setDescription(e.target.value)}
                  className="w-full bg-slate-950 border border-slate-800 text-slate-200 text-sm px-4 py-2.5 rounded-xl focus:outline-none focus:border-indigo-500 h-20 resize-none"
                  placeholder="Details of the replacement parts or mechanic advice..."
                />
              </div>
