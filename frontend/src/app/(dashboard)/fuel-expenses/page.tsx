...
import { Fuel, DollarSign, Calendar, Plus, X, Receipt } from "lucide-react";
    ...
    <div className="space-y-8 select-none">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="glass-panel border border-slate-800 p-6 rounded-2xl flex items-center gap-4">
          ...
          <div>
            <p className="text-xs font-semibold uppercase tracking-wider text-slate-500">Total Fuel Liters</p>
        ...
        <div className="glass-panel border border-slate-800 p-6 rounded-2xl flex items-center gap-4">
          ...
          <div>
            <p className="text-xs font-semibold uppercase tracking-wider text-slate-500">Total Fuel cost</p>
        ...
        <div className="glass-panel border border-slate-800 p-6 rounded-2xl flex items-center gap-4">
          ...
          <div>
            <p className="text-xs font-semibold uppercase tracking-wider text-slate-500">Tolls & Expenses</p>
            <h3 className="text-xl font-bold font-mono text-slate-100 mt-1">{formatCurrency(totalExpenseCost)}</h3>
          </div>
        </div>
      </div>
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="border-b border-slate-800 bg-slate-900/30">
                  ...
                  <th className="p-4 text-xs font-semibold uppercase tracking-wider text-slate-400">Fuel Purchase Date</th>
                  <th className="p-4 text-xs font-semibold uppercase tracking-wider text-slate-400">Liters (L)</th>
                  <th className="p-4 text-xs font-semibold uppercase tracking-wider text-slate-400">Cost</th>
                  <th className="p-4 text-xs font-semibold uppercase tracking-wider text-slate-400 text-right">Avg Unit Cost</th>
                </tr>
              </thead>
                  ...
                  fuelLogs.map((log) => {
                    ...
                    return (
                      <tr key={log.id} className="hover:bg-slate-900/30 transition-colors">
                        ...
                        <td className="p-4 font-mono text-slate-200 text-sm">{formatNumber(log.liters)} L</td>
                        <td className="p-4 font-mono text-emerald-400 font-bold text-sm">{formatCurrency(log.cost)}</td>
                        <td className="p-4 font-mono text-slate-400 text-sm text-right">
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="border-b border-slate-800 bg-slate-900/30">
                  ...
                  <th className="p-4 text-xs font-semibold uppercase tracking-wider text-slate-400 text-right">Amount</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-850">
                {expenses.length === 0 ? (
                  ...
                  expenses.map((exp) => {
                    ...
                    return (
                      <tr key={exp.id} className="hover:bg-slate-900/30 transition-colors">
                        ...
                        <td className="p-4 font-mono text-slate-300 text-sm">{exp.expense_date}</td>
                        <td className="p-4 font-semibold text-slate-200 text-sm">{exp.type}</td>
                        <td className="p-4 font-mono text-emerald-400 font-bold text-sm text-right">{formatCurrency(exp.amount)}</td>
                      </tr>
                    );
                  })
                )}
          ...
          <div className="relative w-full max-w-lg bg-slate-900 border border-slate-800 rounded-3xl p-8 shadow-2xl flex flex-col relative overflow-hidden animate-slide-in">
            ...
            <form onSubmit={handleCreateFuel} className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="space-y-1.5">
                  <label className="text-xs font-semibold text-slate-400 uppercase tracking-wider">Total Cost ($)</label>
                  <input
                    type="number"
                    value={fuelCost}
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1.5">
                  ...
                  >
                    ...
                    <option className="Permits">Permits</option>
